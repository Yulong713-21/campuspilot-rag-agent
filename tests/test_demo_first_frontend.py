...
class PiaFirstFrontendTest(unittest.TestCase):
    def test_homepage_is_pia_workspace_with_embedded_verified_workflow(self) -> None:
        ...
        self.assertIn('class="starter-grid"', response.text)
        self.assertIn("课程与规则", response.text)
        self.assertIn("申请建议", response.text)
        self.assertIn("项目比较", response.text)
        self.assertNotIn('class="workflow-proof"', response.text)
