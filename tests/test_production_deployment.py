...
class ProductionDeploymentTest(unittest.TestCase):
    def test_compose_keeps_fastapi_private_and_low_resource(self) -> None:
        ...
        self.assertIn('container_name: campuspilot-postgres', compose)
        self.assertIn('container_name: campuspilot-elasticsearch', compose)
        self.assertNotIn('"5432:5432"', compose)
        self.assertNotIn('"9200:9200"', compose)
        self.assertIn('campuspilot_postgres_data:', compose)
        self.assertIn('campuspilot_elasticsearch_data:', compose)
    ...
    def test_release_scripts_enforce_health_and_rollback(self) -> None:
        ...
        self.assertIn('pg_dump -U campuspilot', deploy)
        self.assertIn('agent alembic upgrade head', deploy)
        self.assertIn('scripts/seed_campuspilot_domain.py', deploy)
        self.assertIn('scripts/smoke_postgres_rules.py', deploy)
        self.assertIn('scripts/build_handbook_lexical_index.py --recreate', deploy)
