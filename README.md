...
当前公开页面采用 Demo-first：只展示已经形成稳定闭环的 Monash C6001 Planner。

- 一键运行 Entry Level 2 推荐场景；
- 自定义 Entry Level、Study Stream、开始学期、负荷和已修课程；
- 生成最快完成、负荷均衡、保留弹性三套毕业路径；
- 展示学分、先修、开课学期、工作负荷和 Capstone 校验；
- 将命中的 Handbook、Unit 与 Course Map 证据显示为可打开的卡片；
- Planner 不依赖 LLM，云模型不可用时仍能完成确定性规划。

Admission、Program Compare、Pia 与院校目录后端接口继续保留，待 Planner 页面完成部署
验收后再按相同可靠性标准逐页迁移。
